...
首次安装 Python 包以及首次加载 faster-whisper、PaddleOCR 模型可能从公开模型仓库下载文件，会消耗网络和磁盘，但不是付费推理。需要完全离线时先准备本地模型缓存，并在视频命令中加 `--offline`。
