...
def scene_timestamps(scenes: list[dict[str, Any]], limit: int) -> list[float]:
    selected = evenly_spaced(scenes, max(1, limit))
    return [round((scene["start"] + scene["end"]) / 2.0, 3) for scene in selected]
...
def process_video(
    ...
    try:
        keyframes, frame_status, frame_error = extract_frames_with_fallback(
            path,
            scene_timestamps(scenes, max(1, args.max_keyframes)),
            keyframe_dir,
            "keyframe",
        )
    ...
    required_stage_names = ["probe", "scenes", "keyframes"]
    ...
    analysis = {
        ...
        "limitations": [
            ...
            "Scene and audio metrics require agent review to infer meaning, camera intent, and music structure.",
        ],
    }
