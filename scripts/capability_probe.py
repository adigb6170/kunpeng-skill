...
PROFILES = ("all", "video", "image", "document")
...
MODULES = {
    "faster_whisper": ("faster-whisper", "audio transcription", ("video",), ("video",)),
    ...
    "librosa": ("librosa", "audio rhythm and silence metrics", ("video",), ("video",)),
    "numpy": ("numpy", "numeric processing", ("video", "image"), ("video", "image")),
    ...
    "image": (
        {
            ...
            "fallback": "Inspect the original image with host vision and report that metrics are missing.",
            "host_capability": "vision",
        },
        {
...
def selected_profiles(profile: str) -> tuple[str, ...]:
    return ("video", "image", "document") if profile == "all" else (profile,)
            ...
            routes.append(
                {
                    ...
                    "route": "standard" if not missing else "agent_check_required",
                    "standard_components": list(definition["components"]),
                    "missing_components": missing,
                    "fallback": None if not missing else definition["fallback"],
                    "host_capability": None if not missing else definition["host_capability"],
                }
            )
