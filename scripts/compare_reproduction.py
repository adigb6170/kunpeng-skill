...
def compare_images(reference: Path, candidate: Path, mode: str) -> dict[str, Any]:
    ...
    return {
        "diagnostic_score": weighted_score(metrics, weights),
        "dimensions": {
...
def compare_videos(reference: Path, candidate: Path, mode: str, samples: int) -> dict[str, Any]:
    ...
    metrics = {
        ...
        "sampled_motion_rhythm": sequence_similarity(left["motion"], right["motion"]),
    }
    weights = (
        {
            "aspect_ratio": 1.0, "duration": 1.5, "frame_rate": 0.5,
            "color_distribution": 1.0, "brightness": 0.7, "contrast": 0.7,
            "edge_density": 0.7, "sampled_layout": 2.0, "sampled_motion_rhythm": 1.5,
        }
        if mode == "faithful"
        else {
            "aspect_ratio": 0.7, "duration": 0.0, "frame_rate": 0.3,
            "color_distribution": 1.8, "brightness": 1.0, "contrast": 1.0,
            "edge_density": 1.0, "sampled_layout": 0.5, "sampled_motion_rhythm": 1.7,
        }
    )
    return {
        "diagnostic_score": weighted_score(metrics, weights),
...
def compare_texts(reference: Path, candidate: Path, mode: str) -> dict[str, Any]:
    ...
    weights = (
        {
            "sentence_length": 1.0, "paragraph_length": 1.0, "lexical_diversity": 0.8,
            "question_share": 0.5, "exclamation_share": 0.5, "heading_share": 0.6,
            "list_share": 0.6, "punctuation_pattern": 1.0, "length": 0.7,
            "surface_content_overlap": 2.0,
        }
        if mode == "faithful"
        else {
            "sentence_length": 1.4, "paragraph_length": 1.2, "lexical_diversity": 1.0,
            "question_share": 0.8, "exclamation_share": 0.8, "heading_share": 1.0,
            "list_share": 1.0, "punctuation_pattern": 1.2, "length": 0.3,
...
def main() -> int:
    ...
    report = {
        ...
        "mode": args.mode,
        "local_only": True,
        "score_meaning": "A deterministic diagnostic, not proof of semantic or perceptual equivalence.",
        **result,
    }
    if args.output:
